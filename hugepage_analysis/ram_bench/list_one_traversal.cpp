...
#define MIN_ELEMENTS_FACTOR 6
#define MAX_ELEMENTS_FACTOR 20
#define SEQ 0
#define RANDOM 1
...
double time_shuffle(Int N, Int iters) {
...
int main(int argc, const char * argv[])
{
    ...
    Int minElemensFactor = MIN_ELEMENTS_FACTOR;  // First measurement is 2^this number of elements.
    Int maxElemsFactor = MAX_ELEMENTS_FACTOR; // Last measurement is 2^this number of elements. 30 == 16GB of memory
    ...
    Int size = stopsPerFactor * MIN_ELEMENTS_FACTOR; 
    if(argc > 2) {
        size = stopsPerFactor * atoi(argv[2]); 
    }
    ...
    if(type == SEQ) {
        ...
        try {
            Int N = (Int)round(pow(2.0, double(size) / stopsPerFactor));
            Int reps = (Int)round(2e10 / pow(N, 1.5));
            if (reps<1) reps = 1;
            auto ans = time(N, reps);
            cout << (N*sizeof(Node)) << "   " << ans << "   # (N=" << N << ", reps=" << reps << ") " << endl;
        } catch (exception& e) {
